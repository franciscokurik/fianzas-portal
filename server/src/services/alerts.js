...
function marcarNotificado(clientId, tipo, refKey, mensaje) {
  try {
    db.prepare(
...
export async function correrAlertas() {
  const clientes = db.prepare(`SELECT id, email, razon_social FROM clients WHERE role = 'client'`).all();
  let enviadas = 0;
  ...
  for (const c of clientes) {
    ...
    for (const f of fianzas) {
      ...
      if (d !== null && d >= 0 && d <= 30) {
        ...
        if (marcarNotificado(c.id, 'fianza_30', `fianza:${f.id}`, msg)) {
    ...
    for (const doc of docs) {
      ...
      if (d !== null && d >= 0 && d <= doc.alerta_dias) {
        ...
        if (marcarNotificado(c.id, tipo, `doc:${doc.id}:${doc.vencimiento}`, msg)) {
    ...
    for (const p of papeleria) {
      ...
      if (marcarNotificado(c.id, 'papeleria', `papeleria:${p.id}`, msg)) {
